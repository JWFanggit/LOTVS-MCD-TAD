...
class LoadData(object):
    ...
    def __init__(self, mode='train', seq=5, ss=3, w=1280, h=720):  # ss 表示跟踪txt 文件中未记录的帧数
        self.seq = seq
        self.w = w
        self.h = h
        ...
        for image_file in image_files:
            ...
            for i in tqdm.tqdm(os.listdir(image_p)[:-4]):  # i+ss 即表示帧号
                flo_pths = [flow_p + '%06d' % (xx + int(i[:-4]))+ '.png' for xx in range(1, seq - 1)]
